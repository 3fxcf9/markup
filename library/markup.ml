let parse ?(disable_external=false) ?(external_metadata=[]) ?(relative_path="") ?(filename="") ?(file_reader=Fun.const None) ?(file_writer= fun _ _ -> (Error "")) input =
  ...
  let reg: Types.registry = {parsers=ref parsers; head=ref ""; metadata=ref []; debug= ref false; external_metadata; relative_path=ref relative_path; filename=ref filename; depth=0; file_reader; file_writer} in
  ...
  (Printf.sprintf {|<!DOCTYPE html><html><head><meta charset="UTF-8">%s</head><body>%s</body></html>|} !(reg.head) body), reg.metadata
