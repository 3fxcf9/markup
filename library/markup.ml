let parse ?(disable_external=false) ?(external_metadata=[]) ?(relative_path="") ?(project_files=[]) input =
  (* let parsers_content = *)
  ...
  let reg: Types.registry = {parsers; head=""; metadata=[]; debug=false; disable_external; external_metadata; relative_path; project_files; depth=0} in
